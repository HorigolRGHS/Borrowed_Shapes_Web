...
export class CommentsService {
  ...
  async create(dto: CreateCommentDto, userId: string) {
    const thread = await this.commentRepository.getEntityManager().findOne(ForumThread, { id: dto.threadId });
    if (!thread) throw new NotFoundException('comments.thread_not_found');

    let parent: ForumComment | null = null;
    if (dto.parentId) {
      parent = await this.commentRepository.findOne({ id: dto.parentId }, { populate: ['threadId'] });
    ...
    const user = await this.commentRepository.getEntityManager().findOne(User, { id: userId });
    ...
    const gp = await this.commentRepository.getEntityManager().findOne(
      GameProfile,
      { userId },
  ...
  async findComments(threadId: string, parentId: string | null, page: number, limit: number, userId?: string) {
    return this.commentRepository.findCommentsForThread(threadId, parentId, page, limit, userId);
  }

  async update(id: string, dto: UpdateCommentDto, userId: string) {
    const comment = await this.commentRepository.findOne({ id }, { populate: ['authorId'] });
  ...
  async vote(commentId: string, userId: string, value: 1 | -1) {
    ...
    const existingRows = await this.commentRepository.getEntityManager().execute(
    ...
    if (existingValue === null) {
      const vote = this.commentRepository.getEntityManager().create(ForumCommentVote, {
        commentId: comment,
        userId: user,
        value: voteValue,
      });
      comment.score = (comment.score || 0) + value;
      await this.commentRepository.getEntityManager().persistAndFlush([vote, comment]);
      return { result: 'voted', score: comment.score, userVote: value };
    }

    if (existingValue === value) {
      await this.commentRepository.getEntityManager().execute(
