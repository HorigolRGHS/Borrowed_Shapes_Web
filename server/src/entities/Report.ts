...
@Entity({ schema: 'web' })
@Index({ name: 'Report_status_createdAt_idx', expression: 'CREATE INDEX "Report_status_createdAt_idx" ON web."Report" USING btree (status, "createdAt" DESC)', properties: ['status', 'createdAt'] })
export class Report {

  @PrimaryKey({ type: 'text', defaultRaw: `(gen_random_uuid())::text` })
  id!: string & Opt;

  @Index({ name: 'Report_reporterId_idx', expression: 'CREATE INDEX "Report_reporterId_idx" ON web."Report" USING btree ("reporterId")' })
  @ManyToOne({ entity: () => User, fieldName: 'reporterId', deleteRule: 'cascade' })
  reporterId!: User;
  ...
  @ManyToOne({ entity: () => User, fieldName: 'handledBy', deleteRule: 'set null', nullable: true })
  handledBy?: User;

  @Property({ nullable: true })
  handledAt?: Date;

  @Index({ name: 'Report_pending_idx', expression: 'CREATE INDEX "Report_pending_idx" ON web."Report" USING btree ("createdAt" DESC) WHERE (status = \'PENDING\'::web."ReportStatus")' })
